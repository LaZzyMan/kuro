...
import GatherIndices from "./GatherIndices";
import GraphConvolution from "./GraphConvolution";
import SqueezedSparseConversion from "./SqueezedSparseConversion";
...
export function changeBackend(backend: "webgl" | "cpu") {
...
export function createModel() {
  ...
  const inputAdj = new SqueezedSparseConversion({ matrixShape: 1514 }).apply(
...
export async function saveModel(
  model: tf.LayersModel,
  name: string,
  mode?: "localstrorage" | "indexeddb" | "downloads"
) {
  if (!mode) {
    mode = "downloads";
  }
  const result = await model.save(mode + "://" + name);
  return result;
}

export async function loadModel(path: string) {
  const model = await tf.loadLayersModel(path, { strict: true });
  return model;
}
